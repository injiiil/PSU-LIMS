                            ...
                            <div class="span12">
                                <form action="delete_user_log.php" method="post">
                                    <table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
                                        ...
                                        <tbody>
                                            <?php
                                            $user_query = mysqli_query($conn, "SELECT * FROM user_log ORDER BY login_date DESC, logout_date DESC") or die(mysqli_error($conn));
                                            while ($row = mysqli_fetch_assoc($user_query)) {
                                                ?>
                                                <tr>
                                                    <td width="70">
                                                        <input id="optionsCheckbox" class="uniform_on" name="selector[]" type="checkbox" value="<?php echo $row['user_log_id']; ?>">
                                                    </td>
                                                    <td><i class="icon-calendar"></i>&nbsp;<?php echo $row['login_date']; ?></td>
                                                    <td><i class="icon-calendar"></i>&nbsp;<?php echo $row['logout_date']; ?></td>
                                                    <td><i class="icon-user"></i>&nbsp;<?php echo $row['username']; ?></td>
                                                </tr>
                                                <?php
                                            }
                                            ?>
